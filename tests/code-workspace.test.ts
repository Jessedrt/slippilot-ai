...
const market = (
  ...
  sport: 'football',
  category: 'goals',
  marketName: `Total ${marketId}`,
  selectionName: `Over ${selectionId}`,
  odds,
  status: 'active',
  lastUpdated: now,
});
...
function dependencies() {
  ...
  const prices = [
    market('e1', 'm1', 's1', 1.45),
    market('e1', 'm2', 's2', 1.85),
    market('e2', 'm3', 's3', 1.65),
  ];
  const createCode = vi
    .fn<SportyBetProvider['createBookingCode']>()
    .mockResolvedValue('NEWCODE123');
  const provider: SportyBetProvider = {
    name: 'SportyBet',
    listEvents: () => Promise.resolve(all),
    findEvents: () => Promise.resolve(all),
    ...
    resolveBookingCode: () => Promise.resolve(selections),
    createBookingCode: createCode,
    health: () => Promise.resolve({ ok: true, detail: 'test' }),
  };
  const analyze = vi.fn((items: NormalizedMarket[]) =>
    Promise.resolve({
      model: 'test',
      analyzedAt: now.toISOString(),
      summary: 'Every market verified.',
  ...
  it('authenticates import, compares actual options, chooses a specific market, trims and creates a new code', async () => {
    ...
    try {
      ...
      const options = await app.inject({
        ...
        headers,
        payload: { eventId: 'e1', sport: 'football' },
      });
      expect(options.statusCode).toBe(200);
      expect(
        options
          .json<{ options: Array<{ marketId: string }> }>()
          .options.map((item) => item.marketId),
      ).toEqual(['m1', 'm2']);
      const forged = await app.inject({
        ...
        payload: {
          ...
          marketId: 'm2',
          selectionId: 's2',
          selections: importedData.editableSlip.selections.map((item) => ({
            ...item,
            eventId: 'forged',
          })),
      ...
      const trimmed = await app.inject({
        ...
        payload: {
          action: 'reanalyze',
          selections: changedData.selections.slice(0, 1),
          analysisToken: changedData.analysisToken,
        },
      });
