...
describe('AUREX desk routes', () => {
  it('requires Telegram auth, shows only real fixtures, refreshes watchlist and signs reanalyzed edits', async () => {
    ...
    const markets: NormalizedMarket[] = [
      {
        providerMarketId: 'm-1',
        providerSelectionId: 's-1',
        eventId: 'ev-1',
        sport: 'football',
        category: 'total',
        marketName: 'Goals',
      ...
      {
        providerMarketId: 'm-2',
        providerSelectionId: 's-2',
        eventId: 'ev-1',
        sport: 'football',
        category: 'winner',
    ...
    const dependencies = {
      ...
      slipAnalyzer: {
        analyze: (selections: NormalizedMarket[]) => {
          aiCalls += 1;
    ...
    expect(
      fixtures.json<{ fixtures: Array<{ id: string }> }>().fixtures.map((item) => item.id),
    ).toEqual(['ev-1']);
    const watched = await app.inject({
      method: 'POST',
      url: '/api/miniapp/watchlist-refresh',
      headers,
      payload: { ids: ['ev-1', 'not-found'] },
    });
    expect(watched.json<{ results: Array<{ available: boolean }> }>().results).toMatchObject([
      { available: true },
      { available: false },
    ]);
    ...
    const original = build.json<{
      selections: Array<{ selectionId: string; marketId: string }>;
    ...
    const tampered = await app.inject({
      ...
      payload: {
        action: 'replace',
        index: 0,
        selections: [{ ...original.selections[0], selectionId: 'fabricated' }],
    ...
    const changed = replacement.json<{
      selections: Array<{ marketId: string }>;
      analysisToken: string;
    }>();
    ...
    const reanalysis = await app.inject({
      ...
      payload: {
        ...
        analysisToken: changed.analysisToken,
      },
    });
