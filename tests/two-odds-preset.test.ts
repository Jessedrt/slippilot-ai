...
const option = (id: number, hour: number, odds: number, confidence: number,
  verdict: 'keep' | 'caution' | 'reject' = 'keep', risk: 'lower' | 'medium' | 'higher' = 'lower') => {
  const base = candidate(id, odds);
  return {
    candidate: { ...base, fixture: { ...base.fixture,
...
beforeEach(() => { vi.useFakeTimers(); vi.setSystemTime(new Date('2026-09-20T05:00:00Z')); });
...
describe('conservative 2.00 odds preset', () => {
  it('reviews the whole day, ranks stronger evidence and spans morning and evening windows', () => {
    const picks = selectTwoOddsPicks([
      option(1, 8, 1.3, 58), option(2, 8, 1.29, 94),
      option(3, 8, 1.31, 90), option(4, 20, 1.27, 91),
      option(5, 20, 1.28, 89),
    ], 3, Math.pow(2, 1 / 3));
  ...
  it('uses only verified lower-risk outcomes and returns a shortfall rather than forcing 2.00', async () => {
    ...
    const provider: SportyBetProvider = {
      ...
      getEvent: (id) => Promise.resolve(events.find((event) => event.providerEventId === id) ?? null),
      getMarkets: (eventId) => Promise.resolve([{
        eventId, providerMarketId: `market-${eventId}`, providerSelectionId: `outcome-${eventId}`,
        sport: 'football', category: 'winner', marketName: 'Winner', selectionName: 'Home',
        odds: 1.24, status: 'active', lastUpdated: new Date(),
      }]),
      refreshSelections: (selections) => Promise.resolve(selections.filter((selection) =>
        selection.eventId !== 'event-2')),
    ...
    const result = await buildReviewedLiveSlipSnapshot(provider, analyzer, 'football', 3, 2, 'conservative');
