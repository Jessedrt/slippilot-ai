...
function setup(confidence = 55, valid = true) {
  const analyze = vi.fn((items: unknown[]) =>
    Promise.resolve({
      model: 'test',
      analyzedAt: now.toISOString(),
      summary: 'Actual provider data reviewed.',
  ...
  return {
    deps: { sportyBet: provider, slipAnalyzer: { analyze }, telegramBotToken: token },
    analyze,
  };
}
...
describe('booking code import recovery', () => {
  it('discloses started legs, analyzes only current markets and signs 55 minimum', async () => {
    ...
    const claim = z
      .object({ minimumScore: z.number(), selections: z.array(z.string()) })
      .parse(
        JSON.parse(
          Buffer.from(result.editableSlip!.analysisToken.split('.')[0]!, 'base64url').toString(
            'utf8',
          ),
        ) as unknown,
      );
  ...
  it('returns higher-scored verified legs first, including in the signed editable slip', async () => {
    ...
    vi.setSystemTime(now);
    const { deps, analyze } = setup();
    vi.spyOn(deps.sportyBet, 'resolveBookingCode').mockResolvedValue([
      pick('lower-score'),
      pick('higher-score'),
    ]);
    analyze.mockImplementation((items) =>
      Promise.resolve({
        model: 'test',
        analyzedAt: now.toISOString(),
        summary: 'Ranked.',
    ...
    const result = await importBookingCode('ABCD1234', deps, 'verified-init-data');
    expect(result.selections.map((item) => item.eventId)).toEqual(['higher-score', 'lower-score']);
    expect(result.editableSlip?.selections.map((item) => item.eventId)).toEqual([
      'higher-score',
      'lower-score',
    ]);
    const tokenPayload = JSON.parse(
      Buffer.from(result.editableSlip!.analysisToken.split('.')[0]!, 'base64url').toString('utf8'),
  ...
  it('returns actionable conflict rather than HTTP 500 when all legs have started', async () => {
    ...
    await expect(importBookingCode('ABCD1234', deps, 'verified-init-data')).rejects.toMatchObject({
      statusCode: 409,
    });
