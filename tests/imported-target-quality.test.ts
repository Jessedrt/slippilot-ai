...
const sportyBet: SportyBetProvider = {
  ...
  getEvent: (id) => Promise.resolve({ providerEventId: id, homeTeam: `Home ${id}`,
    awayTeam: `Away ${id}`, startsAt: start, status: 'scheduled' }),
  getMarkets: (id) => Promise.resolve([{ eventId: id, providerMarketId: 'm',
    providerSelectionId: 's', sport: 'football', category: 'goals',
    marketName: 'Total', selectionName: 'Over 1.5', odds: 1.5,
...
const slipAnalyzer: SlipAnalyzer = {
  analyze: (candidates) => Promise.resolve({
    model: 'mock', analyzedAt: new Date().toISOString(), summary: 'Provider markets checked.',
    selections: candidates.map((pick, index) => ({ index: index + 1,
      confidence: pick.eventId === 'lower' ? 60 : 85,
      risk: 'lower' as const, verdict: 'keep' as const, reason: 'Reviewed.' })),
  }),
};
...
describe('strict imported-code targets preserve score policy', () => {
  it.each([2, 5])('requires 68 for %i odds, then keeps that minimum signed', async (targetOdds) => {
    ...
    const payload = { action: 'reanalyze', riskMode: 'balanced',
      analysisToken: original.analysisToken, targetOdds,
      selections: original.selections };
    const rejected = await app.inject({ method: 'POST', url: '/api/miniapp/edit-slip',
      headers: { 'x-telegram-init-data': initData }, payload });
    expect(rejected.statusCode).toBe(409);
    expect(rejected.json<{ message: string }>().message).toContain('68/100');
    const accepted = await app.inject({ method: 'POST', url: '/api/miniapp/edit-slip',
      headers: { 'x-telegram-init-data': initData },
      payload: { ...payload, selections: original.selections.filter((pick) => pick.confidence >= 68) } });
    expect(accepted.statusCode).toBe(200);
    const approved = accepted.json<{ qualityMinimum: number; analysisToken: string;
    ...
    const attemptedLowering = await app.inject({ method: 'POST', url: '/api/miniapp/edit-slip',
      headers: { 'x-telegram-init-data': initData },
      payload: { ...payload, targetOdds: 10,
        selections: approved.selections, analysisToken: approved.analysisToken } });
