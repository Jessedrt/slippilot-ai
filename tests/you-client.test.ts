...
const searchPayload = {
  results: {
    web: [{ url: 'https://www.bbc.com/sport/football/a', title: 'Team news', description: 'Player is fit to play.', snippets: ['Player is fit to play.'], page_age: '2026-09-15T10:00:00Z' }],
    news: [],
  },
...
describe('YouClient', () => {
  it('sends Search to the official endpoint with X-API-Key and parses results', async () => {
    ...
    const result = await new YouClient({ apiKey: 'secret-key', fetch: fetchMock, sleep: () => Promise.resolve() }).search('team news');
    ...
    expect(JSON.parse(bodyText(calls[0]?.init?.body)) as unknown).toMatchObject({ query: 'team news' });
  });

  it('parses Answer, Research, and Contents response shapes', async () => {
    const replies = [
      response({ answer: 'Available [[1]]', citations: [{ source: 'https://nba.com/a', excerpts: ['Available'] }], results: { web: [] } }),
      response({ output: { content: 'Research summary', sources: searchPayload.results.web } }),
      response([{ url: 'https://nba.com/a', title: 'NBA', markdown: '# Update', metadata: { site_name: 'NBA' } }]),
    ];
    const client = new YouClient({ apiKey: 'key-key-key', fetch: () => Promise.resolve(replies.shift() ?? response({})), sleep: () => Promise.resolve() });
  ...
  it('honors Retry-After for 429 and retries an idempotent request', async () => {
    ...
    const client = new YouClient({
      apiKey: 'key-key-key',
      fetch: () => Promise.resolve(++calls === 1 ? response({}, 429, { 'Retry-After': '1' }) : response(searchPayload)),
  ...
  it('does not retry non-transient API errors', async () => {
    let calls = 0;
    const client = new YouClient({ apiKey: 'key-key-key', fetch: () => { calls += 1; return Promise.resolve(response({}, 401)); } });
    await expect(client.search('news')).rejects.toThrow('HTTP 401');
    expect(calls).toBe(1);
  });

  it('aborts timed-out requests', async () => {
    const fetchMock = ((_input: URL | RequestInfo, init?: RequestInit) => new Promise<Response>((_resolve, reject) => {
      init?.signal?.addEventListener('abort', () => reject(new DOMException('Timed out', 'AbortError')));
    })) as typeof fetch;
    const client = new YouClient({ apiKey: 'key-key-key', fetch: fetchMock, timeoutMs: 5, sleep: () => Promise.resolve() });
  ...
  it('uses cached normalized responses without a second API request', async () => {
    ...
    const cache = {
      ...
      set: <T>(key: string, value: T) => { store.set(key, value); return Promise.resolve(); },
    };
    const client = new YouClient({ apiKey: 'key-key-key', cache, fetch: () => { calls += 1; return Promise.resolve(response(searchPayload)); } });
...
describe('You.com research normalization', () => {
  ...
  it('marks conflicting player availability and reduces confidence', () => {
    const items = [
      { url: 'https://nba.com/a', title: 'Official update', snippets: ['Player ruled out and unavailable.'], page_age: '2026-09-15T11:00:00Z' },
      { url: 'https://espn.com/b', title: 'Reporter update', snippets: ['Player is fit to play and available.'], page_age: '2026-09-15T11:00:00Z' },
    ];
    const result = normalizeSportsResearch('Availability update.', items, 8, new Date('2026-09-15T12:00:00Z'));
  ...
  it('falls back without blocking analysis when You.com is unavailable', async () => {
    ...
    const fixture = { id: '1', sport: 'football' as const, league: 'Premier League', homeTeam: 'Arsenal', awayTeam: 'Chelsea', startsAt: new Date(), status: 'scheduled' as const };
    const result = await new SportsResearchService(unavailable).researchFixture(fixture, ['injury_uncertainty']);
  ...
  it('skips You.com when no freshness trigger is present', async () => {
    ...
    const provider: WebResearchProvider = {
      search: () => { called = true; return Promise.resolve(searchPayload); },
    ...
    const fixture = { id: '1', sport: 'football' as const, league: 'Premier League', homeTeam: 'Arsenal', awayTeam: 'Chelsea', startsAt: new Date(), status: 'scheduled' as const };
    expect((await new SportsResearchService(provider).researchFixture(fixture, [])).status).toBe('skipped');
