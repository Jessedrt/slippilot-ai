...
export const envSchema = z
  .object({
    ...
    // Legacy declaration retained for compatibility. It is not proof of data rights.
    API_SPORTS_COMMERCIAL_USE_APPROVED: z.stringbool().default(false),
    API_SPORTS_DATA_RIGHTS_CONFIRMED: z.stringbool().default(false),
    API_SPORTS_FOOTBALL_ENABLED: z.stringbool().default(false),
    API_SPORTS_BASKETBALL_TOTALS_ENABLED: z.stringbool().default(false),
    API_SPORTS_VERIFIED_MAPPINGS_JSON: z.preprocess(
      blankToUndefined,
      z.string().max(1_000_000).optional(),
    ),
    ...
    API_SPORTS_ANALYSIS_CONCURRENCY: z.coerce.number().int().min(1).max(5).default(2),
    ANALYSIS_DEADLINE_MS: z.coerce.number().int().min(10_000).max(50_000).default(42_000),
    ...
    YOU_TOTAL_TIMEOUT_MS: z.coerce.number().int().min(5_000).max(40_000).default(25_000),
    YOU_MAX_ATTEMPTS: z.coerce.number().int().min(1).max(3).default(2),
