...
const outcomeSchema = z
  .object({
    id: z.coerce.string(),
    odds: z.coerce.number().nonnegative(),
...
const marketSchema = z
  .object({
    ...
    lastOddsChangeTime: z.coerce.number().optional(),
...
const shareResponseSchema = responseSchema(
  z.object({
    shareCode: z.string(),
...
const bookingResponseSchema = responseSchema(
  z.object({
    shareCode: z.string(),
    shareURL: z.string().optional(),
    deadline: z.union([z.string(), z.number()]).optional(),
...
// Verified current primary-list markets. Full event catalogs are always discovered dynamically.
const primaryMarketIds: Record<Sport, string> = { football: '1', basketball: '219' };

/** Verified against SportyBet Nigeria's public browser client on 2026-09-15. */
export class SportyBetClient {
  ...
  async fetchFixtures(sport: Sport): Promise<SportyBetEvent[]> {
    const raw = await this.cached(`fixtures:${sport}`, async () => {
      const data = await this.request('GET', '/factsCenter/pcUpcomingEvents', {
        ...
        marketId: primaryMarketIds[sport],
        pageSize: '100',
        pageNum: '1',
      });
  ...
  async getMarkets(eventId: string): Promise<NormalizedMarket[]> {
    const event = await this.getRawEvent(eventId);
    return this.normalizeMarkets(event);
  }

  async refreshSelections(selections: ProviderSelection[]): Promise<BookingSelection[]> {
    this.validateSelectionInput(selections);
    const payload = selections.map((selection) => this.toTuple(selection));
    const response = refreshResponseSchema.parse(
      await this.request('POST', '/factsCenter/Outcomes', undefined, payload, false),
    );
    this.assertBusinessSuccess(response);
  ...
  async createBookingCode(selections: ProviderSelection[]): Promise<BookingCodeDetails> {
    ...
    const response = shareResponseSchema.parse(
      await this.request(
        'POST',
        '/orders/share',
        undefined,
        { selections: refreshed.map((selection) => this.toTuple(selection)) },
        false,
      ),
    );
    this.assertBusinessSuccess(response);
    return {
      ...
      currentOdds: this.combinedOdds(refreshed),
  ...
  async getBookingCode(code: string): Promise<BookingCodeDetails> {
    ...
    const response = bookingResponseSchema.parse(
      await this.request('GET', `/orders/share/${encodeURIComponent(code.toUpperCase())}`),
    );
    this.assertBusinessSuccess(response);
    ...
    return {
      ...
      ...(response.data.deadline !== undefined ? { deadline: response.data.deadline } : {}),
      selections,
      currentOdds: this.combinedOdds(selections),
  ...
  async health(): Promise<{ ok: boolean; detail: string }> {
    try {
      const data = await this.request('GET', '/factsCenter/sportList');
      const response = responseSchema(z.unknown()).parse(data);
      this.assertBusinessSuccess(response);
  ...
  private async getRawEvent(eventId: string, useCache = true): Promise<RawEvent> {
    ...
    const load = async () => {
      const response = eventResponseSchema.parse(
        await this.request('GET', '/factsCenter/event', { eventId, productId: '3' }),
      );
      this.assertBusinessSuccess(response);
  ...
  private async validateCurrentSelections(selections: ProviderSelection[]): Promise<void> {
    ...
    for (const selection of selections) {
      ...
      const market = event.markets.find(
        (candidate) =>
          candidate.id === selection.marketId &&
  ...
  private normalizeMarkets(event: RawEvent): NormalizedMarket[] {
    ...
    return event.markets.flatMap((market) => {
      ...
      return market.outcomes.map((outcome) => ({
        ...
        providerSelectionId: outcome.id,
        eventId: event.eventId,
        sport,
        category: market.group ?? 'Other',
        marketName: market.name ?? market.desc ?? market.id,
        selectionName: outcome.desc,
        odds: outcome.odds,
        ...(line !== undefined ? { line } : {}),
    ...
    return events.flatMap((event) => {
      ...
      return event.markets.flatMap((market) =>
        market.outcomes
          .filter(
            (outcome) =>
              outcome.isActive === 1 && outcome.odds > 1 && market.status <= 1 && !market.banned,
          )
          .map((outcome) => ({
            eventId: event.eventId,
            marketId: market.id,
            ...
            odds: outcome.odds,
            ...(market.specifier ? { specifier: market.specifier } : { specifier: null }),
            homeTeam: event.homeTeamName,
            awayTeam: event.awayTeamName,
            marketName: market.name ?? market.desc ?? market.id,
            selectionName: outcome.desc,
            startsAt: new Date(event.estimateStartTime),
            sport,
            tournament: event.sport.category?.tournament.name ?? 'Unknown',
          })),
      );
    });
  }
  ...
  private validateSelectionInput(selections: ProviderSelection[]): void {
    if (selections.length === 0) throw new Error('At least one SportyBet selection is required.');
    ...
    for (const selection of selections) {
      ...
      if (marketScopes.has(scope))
        throw new Error(`Incompatible SportyBet selections from one market: ${scope}`);
  ...
  private sameTuple(left: ProviderSelection, right: ProviderSelection): boolean {
    return (
      ...
      (left.specifier ?? null) === (right.specifier ?? null)
    );
  }
      ...
      try {
        return await this.runLimited(async () => {
          ...
          try {
            ...
            if (!response.ok)
              throw new SportyBetHttpError(`SportyBet HTTP ${response.status}`, response.status);
